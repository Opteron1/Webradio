...
volatile u16 shoutcast_status=SHOUTCAST_CLOSED;
u16 shoutcast_localport=0;
...
{
  s32 timeout;
  u16 idx, trying, status, station_status;
  ...
  switch(status)
  {
    case SHOUTCAST_ADDRMOVED:  station_status = STATION_ADDRMOVED;  break;
    case SHOUTCAST_ERRTIMEOUT: station_status = STATION_ERRTIMEOUT; break;
		default:									 station_status = STATION_ERROR; break;
  }
...
void shoutcast_tcpapp(u16 idx, const unsigned char *rx, u16 rx_len, unsigned char *tx)
{
  u16 tx_len, i;
  char buf[32], *ptr;
  static u16 parse_header=0, format=0, format_header=0, skip=0;
