...
volatile u16 rtsp_status=RTSP_CLOSED;
u16 rtsp_localport=0;
...
u16 rtsp_open(void)
{
  ...
  switch(status)
  {
    case RTSP_ADDRMOVED:  station_status = STATION_ADDRMOVED;  break;
    case RTSP_ERRTIMEOUT: station_status = STATION_ERRTIMEOUT; break;
...
void rtsp_tcpapp(u16 idx, const unsigned char *rx, u16 rx_len, unsigned char *tx)
{
  ...
  static u16 seq=0, content_len=0, pkt_len=0;
  const RTSP_Header *rtsp;

static s32 keepalive=0;
