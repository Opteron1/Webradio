...
#define DEFAULT_IP                     "192.168.001.050"
...
#define UDP_ENTRIES                    (10)//max UDP Table Entries
...
typedef struct
{
  ...
  IP_Addr  dns;
  IP_Addr  ntp;
  s16      timediff; //Time Diff to GMT in seconds
  s16      summer;   //Summer Time
...
#define RX_DESCR_CNT		2U															//temporary
#define TX_DESCR_CNT		2U															//temporary
extern u8 eth_txbuf_temp[TX_DESCR_CNT][ETH_MTUSIZE+6];	//temporary
extern u8 eth_rxfifo_temp[RX_DESCR_CNT][ETH_MTUSIZE+6];	//temporary


void         													 eth_setsummer(s16 on);
void         													 eth_settimediffh(s16 hours);
void         													 eth_settimediff(s16 seconds);
...
s16                                    eth_getsummer(void);
s16                                    eth_gettimediffh(void); //hours
s16                                    eth_gettimediff(void);  //seconds
IP_Addr                                eth_getntp(void);
IP_Addr                                eth_getdns(void);
s16                                    eth_getdhcp(void);
...
void																	 eth_init(void);
void                                   xv_eth_init_ethernet_init(void);				/* Initialize ETH module */
